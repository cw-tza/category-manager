...
function tree (mwCategories, indexFn = indexByNameToLowerCase) {
  ...
  const withRootIds = mwCategories.map(
    mwCat => _.defaults(mwCat, {parentId: ROOT_ID}))

  const topLevel = arrayToTree(withRootIds,
    {parentProperty: 'parentId', rootID: ROOT_ID})

  return _.toArray(topLevel).map(top => indexed(top, indexFn)).reduce(_.merge)
}

function indexed (category, indexFn = indexByNameToLowerCase) {
  const indexChildren = children => children
    .map(c => indexed(c, indexFn))
    .reduce(_.merge, {})
...
function pathAsTree (path) {
  ...
  const branch = [].concat(path.split('/').map(name => ({name: name})).map(
    cat => ({...cat, adi: true})).reduceRight(joinChild)
  )
...
function merge (mwTree, ...pathTrees) {
  ...
  return _.mapValues([...pathTrees, mwTree].reduce(_.merge),
    cat => cascadeData(cat, rootParentData))
  ...
  function cascadeData (category, parentData) {
    ...
    const children = _.chain(category).omit('$').mapValues(
      child => cascadeData(child, $)).value()
...
function search (tree, ...paths) {
  const pathArrays = paths.map(
    path => path.split('/').map(name => name.toLowerCase()))
...
function treeFilter (tree, predicate) {
  ...
  return _.chain(tree).pickBy(predicate).flatMapDeep(childFilter).value()
}
