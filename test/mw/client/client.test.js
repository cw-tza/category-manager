...
const build = require('../../../src/mw/xml/category-xml-writer')
const Client = require('mw/client/client-factory')

describe('mw category client tests', () => {
  let client = new Client('http://mock/', 'token')

  beforeEach(() => mocking.init(client))
  afterEach(() => mocking.get().reset())
  //
  ...
  test('should post when id undefined', async () => {
    ...
    mocking.get().onPost('/categories', build(cat1)).reply(201, '').onPost('/categories', build(cat2)).reply(201, '')
  ...
  test('should put when id is defined', async () => {
    ...
    mocking.get().onPut('/categories/1', build(cat1)).reply(204, '').onPut(
      '/categories/2', build(cat2)).reply(204, '')
