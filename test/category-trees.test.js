...
let setParsed = (parsed) => {
  payloads = _.flatten(parsed)

  return parsed
}

beforeAll(() => testData.init()
  .then(data => Promise.all(data.map(parse))).then(setParsed))
...
describe('category-tree tests', () => {
  ...
  test('should find by path', () => {
    ...
    let searched = catTrees.search(merged, 'Adultos/Latinas/BIZARRE',
      'Adultos/Nuevo', 'x/z/a/s', 'Drama')
